---
name: fluxpoint-migrate
description: Migrate a pre-1.0 repo onto the unified WORK.md contract, via the tested migrate script — plan, apply, verify, then finalize. Codex entry point for the fluxpoint `migrate` command.
user-invocable: false
disable-model-invocation: true
---

This skill is how Codex reaches the `migrate` command of the fluxpoint
plugin; on Claude Code the same command is `/fluxpoint:migrate`, and the two
Claude-only keys above keep this entry point out of that runtime's menus so
nothing is listed twice.

1. Read `../../commands/migrate.md`, relative to this file. Under an installed
   plugin that is `${PLUGIN_ROOT}/commands/migrate.md`; Codex also sets
