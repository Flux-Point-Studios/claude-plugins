...
description: Run the substrate registry check and interpret it — graph size, orphans as dormant value, hubs to harden, staleness alarms and coverage notes — then recommend the single next action. Codex entry point for the substrate `status` command.
user-invocable: false
disable-model-invocation: true
---

This skill is how Codex reaches the `status` command of the substrate
plugin; on Claude Code the same command is `/substrate:status`, and the two
