#!/usr/bin/env python3
"""Compile a GRAPH.md declarative IR block into a Claude Code Workflow script.

The IR is the single source of truth. This compiler is deterministic and has
...
Usage:
  compile-graph.py GRAPH.md -o .claude/workflows/campaign.graph.js
  compile-graph.py GRAPH.md --check      # validate only, emit nothing
...
def extract_ir(md_text):
    ...
    if not blocks:
        raise GraphError(
            "no IR block found — GRAPH.md needs one ```json graph-ir fenced block"
        )
    if len(blocks) > 1:
...
def emit(ir, contracts):
    ...
    L = []
    a = L.append
    a("// GENERATED by fluxpoint-graph compile-graph.py — DO NOT EDIT.")
    a("// Source of truth is the ```json graph-ir block in GRAPH.md.")
    a("// Regenerate with /fluxpoint-graph:graph-run (or compile-graph.py).")
    a("export const meta = {")
...
def emit_node(n, ir):
    ...
    if n.get("foreach"):
        ...
        if panel and over:
            ...
            a("    const judged = await parallel(produced.map(it => () =>")
            a(f"      refute(JSON.stringify(it), `{nid}:${{item.key || i}}`, {js_str(phase)}, {panel})")
            a("        .then(v => ({ ...it, source: item.key || String(i), ...v }))")
            a("    ))")
            need = math.ceil(panel / 2)
            a(f"    return judged.filter(Boolean).filter(v => v.unverified || v.kills < {need})")
            a("  }")
        ...
        label = f"{js_str(nid)}"
        a(f"const {var} = await agent({prompt}, {opts(n, ir, phase, label)})")
...
def main():
    ...
    ap.add_argument("graph", help="path to GRAPH.md")
