...
before red-team sees it. Copy over `GRAPH.md` to use it, or keep both and
point `graph-run` at this file.

Requires fluxpoint-loop: the gate resolves `red-team-reviewer` via
...
| builder | implementation | inline prompt, worktree-isolated | works a fluxpoint-loop slice |
...
```json graph-ir
{
  ...
  "requiredArgs": ["goal"],
  "argDefaults": { "constraints": "none beyond LOOP.md" },
  "roles": {
  ...
  "nodes": [
    ...
    },
    {
      ...
      "phase": "Council",
      "after": "design",
      "prompt": "Judge these candidate designs side by side for the goal \"{{A.goal}}\" on TDD-ability, blast radius, fit with the Definition of Done in LOOP.md, and honesty of their risks. Candidates: {{prev}}. Return the single strongest design, grafting in the best ideas from the runners-up. Barrier justified: judging requires all candidates at once.",
      "contract": "DesignV1",
      "effort": "high",
      "verify": "schema-only",
      "onRed": "halt"
...
- Terminal: merge happens outside the graph, per LOOP.md Merge policy,
