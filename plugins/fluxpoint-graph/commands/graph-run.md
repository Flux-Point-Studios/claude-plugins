---
description: Compile GRAPH.md's IR to a Workflow script, execute it, record provenance and the Evidence row — targeted node repair and cached resume on partial failure, never a restart from zero.
argument-hint: [path to GRAPH.md, plus any campaign args as JSON]
---
...
1. Resolve the plugin root: try `${CLAUDE_PLUGIN_ROOT}`, else
   ...
   The graph file is "$ARGUMENTS" if it names one, else `GRAPH.md`.
2. Preflight, all deterministic — stop and report exactly what is missing
   ...
   - Every command the verification map names exists (`scripts/harness.sh`
     ...
     requires fluxpoint-loop installed.
3. Compile:
