...
| Metrics | structured provenance, `scripts/metrics.py`, `/fluxpoint:status` | Every summary records agent calls spawned vs planned and the runtime's token meter; discovery rounds carry found/new/kept tallies with per-worker unique-new counts; reduces carry before/after. `metrics.py` folds runs, lessons, and inbox into per-campaign rates — death and skip rates, dry-vs-ceiling endings, panel kill rate — so campaigns are tuned against numbers, not vibes. |
